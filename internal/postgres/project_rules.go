...
// and disabled, across every source archive mirrored into this PG store,
// each annotated with its current governed-session count. It mirrors
// internal/db.ListProjectRules with PG's multi-archive shape: unlike
// SQLite (one archive, so one rule per (machine, path_prefix)), two
// archives can each have their own rule for the same (machine, path_prefix)
// pair, and both appear here with their own SourceArchiveID and their own
// governed count, keyed by db.GovernedRuleKey{SourceArchiveID, Machine,
// PathPrefix}. Rows are ordered by (path_prefix, source_archive_id); machine
// is constant across the result set since it is the filter. The machine
// list keeps the typeahead contract of internal/db.ListProjectRules: every
// machine with a live session or a stored mapping, across every archive,
...
func (s *Store) ListProjectRules(ctx context.Context, machine string) (db.ProjectRules, error) {
	...
	if err != nil {
		return db.ProjectRules{}, fmt.Errorf("listing pg project rule machines: %w", err)
	}
...
func (s *Store) projectRulesMachines(ctx context.Context) ([]string, error) {
	rows, err := s.pg.QueryContext(ctx, `
		...
		ORDER BY machine`)
	if err != nil {
		return nil, fmt.Errorf("listing pg project rule machines: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&machine); err != nil {
			return nil, fmt.Errorf("scanning pg project rule machine: %w", err)
		}
		machines = append(machines, machine)
	}
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("iterating pg project rule machines: %w", err)
	}
	return machines, nil
}

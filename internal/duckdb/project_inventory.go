...
// archive mirrored into this DuckDB store into a per-project inventory,
...
// to internal/db's SQLite version and internal/postgres's PG version,
// including cross-platform cwd normalization; no source_archive_id filter,
// since visibility (unlike governedness) does not depend on provenance.
// Unlike SQLite's TEXT timestamps, the mirror's started_at/ended_at are
// typed TIMESTAMP columns and the push path (nilTime) converts legacy
// empty-string values to NULL, so no NULLIF guard is needed here (same
// reasoning as PG; NULLIF against an empty string literal would in fact
...
// parseDuckInventoryTime converts a scanned MIN/MAX(TIMESTAMP) value into a
// *time.Time, treating NULL (an empty formatted string) as "no bound".
...
func (s *Store) projectInventoryVisibleArchives(
	...
	if err != nil {
		return nil, fmt.Errorf("listing duckdb visible source archives: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&archiveID); err != nil {
			return nil, fmt.Errorf("scanning duckdb visible source archive: %w", err)
		}
		out[archiveID] = struct{}{}
	}
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("iterating duckdb visible source archives: %w", err)
	}
	return out, nil
}
...
// archive pushed into this DuckDB store.
...
func (s *Store) projectInventoryMappings(
	...
	if err != nil {
		return nil, nil, fmt.Errorf("listing duckdb worktree mappings: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			return nil, nil, fmt.Errorf("scanning duckdb worktree mapping: %w", err)
		}
	...
	if err := rows.Err(); err != nil {
		return nil, nil, fmt.Errorf("iterating duckdb worktree mappings: %w", err)
	}
